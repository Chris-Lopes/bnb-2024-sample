import React from "react";

const Sponsers = () => {
  return (
    <div className="flex flex-col justify-center items-center w-full ">
      <h1 className="text-8xl mb-20">Our Sponsers</h1>
      <div>
        <div className="text-center mb-10">
          <h2 className="relative inline-flex h-full overflow-hidden rounded-xl p-[2px] mb-4 focus:outline-none focus:ring-2 focus:ring-[#df4173] focus:ring-offset-2 focus:ring-offset-[#df4173]">
            <span className="absolute inset-[-1000%] animate-[spin_2s_linear_infinite] bg-[conic-gradient(from_90deg_at_50%_50%,#880c1d_0%,#89898d_50%,#880c1d_100%)]" />
            <span className="inline-flex h-full w-full cursor-default items-center justify-center rounded-xl bg-[#09090e] px-6 py-3 text-5xl font-medium text-white backdrop-blur-3xl">
              TITLE
            </span>
          </h2>
          <p className="text-2xl"> Yet To Come</p>
        </div>
        <div className="text-center mb-10">
          <h2 className="relative inline-flex h-full overflow-hidden rounded-xl p-[2px] mb-4 focus:outline-none focus:ring-2 focus:ring-[#df4173] focus:ring-offset-2 focus:ring-offset-[#df4173]">
            <span className="absolute inset-[-1000%] animate-[spin_2s_linear_infinite] bg-[conic-gradient(from_90deg_at_50%_50%,#880c1d_0%,#89898d_50%,#880c1d_100%)]" />
            <span className="inline-flex h-full w-full cursor-default items-center justify-center rounded-xl bg-[#09090e] px-6 py-3 text-5xl font-medium text-white backdrop-blur-3xl">
              ASSOCIATE
            </span>
          </h2>
          <p className="text-2xl"> Yet To Come</p>
        </div>
        <div className="text-center mb-10">
          <h2 className="relative inline-flex h-full overflow-hidden rounded-xl p-[2px] mb-4 focus:outline-none focus:ring-2 focus:ring-[#df4173] focus:ring-offset-2 focus:ring-offset-[#df4173]">
            <span className="absolute inset-[-1000%] animate-[spin_2s_linear_infinite] bg-[conic-gradient(from_90deg_at_50%_50%,#880c1d_0%,#89898d_50%,#880c1d_100%)]" />
            <span className="inline-flex h-full w-full cursor-default items-center justify-center rounded-xl bg-[#09090e] px-6 py-3 text-5xl font-medium text-white backdrop-blur-3xl">
              POWERED BY
            </span>
          </h2>
          <p className="text-2xl"> Yet To Come</p>
        </div>
        <div className="text-center mb-10">
          <h2 className="relative inline-flex h-full overflow-hidden rounded-xl p-[2px] mb-4 focus:outline-none focus:ring-2 focus:ring-[#df4173] focus:ring-offset-2 focus:ring-offset-[#df4173]">
            <span className="absolute inset-[-1000%] animate-[spin_2s_linear_infinite] bg-[conic-gradient(from_90deg_at_50%_50%,#880c1d_0%,#89898d_50%,#880c1d_100%)]" />
            <span className="inline-flex h-full w-full cursor-default items-center justify-center rounded-xl bg-[#09090e] px-6 py-3 text-5xl font-medium text-white backdrop-blur-3xl">
              PLATINUM
            </span>
          </h2>
          <p className="text-2xl"> Yet To Come</p>
        </div>
        <div className="text-center mb-10">
          <h2 className="relative inline-flex h-full overflow-hidden rounded-xl p-[2px] mb-4 focus:outline-none focus:ring-2 focus:ring-[#df4173] focus:ring-offset-2 focus:ring-offset-[#df4173]">
            <span className="absolute inset-[-1000%] animate-[spin_2s_linear_infinite] bg-[conic-gradient(from_90deg_at_50%_50%,#880c1d_0%,#89898d_50%,#880c1d_100%)]" />
            <span className="inline-flex h-full w-full cursor-default items-center justify-center rounded-xl bg-[#09090e] px-6 py-3 text-5xl font-medium text-white backdrop-blur-3xl">
              GOLD
            </span>
          </h2>
          <p className="text-2xl"> Yet To Come</p>
        </div>
        <div className="text-center mb-10">
          <h2 className="relative inline-flex h-full overflow-hidden rounded-xl p-[2px] mb-4 focus:outline-none focus:ring-2 focus:ring-[#df4173] focus:ring-offset-2 focus:ring-offset-[#df4173]">
            <span className="absolute inset-[-1000%] animate-[spin_2s_linear_infinite] bg-[conic-gradient(from_90deg_at_50%_50%,#880c1d_0%,#89898d_50%,#880c1d_100%)]" />
            <span className="inline-flex h-full w-full cursor-default items-center justify-center rounded-xl bg-[#09090e] px-6 py-3 text-5xl font-medium text-white backdrop-blur-3xl">
              SILVER
            </span>
          </h2>
          <p className="text-2xl"> Yet To Come</p>
        </div>
      </div>
      <h1 className="text-8xl mb-20">Previous Sponsers</h1>
      <div>
        companies...
      </div>
    </div>
  );
};

export default Sponsers;
